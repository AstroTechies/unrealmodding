...
use unreal_asset::{engine_version::EngineVersion, error::Error, unversioned::Usmap, Asset};
...
const USMAP: &[u8] = include_bytes!(concat!(assets_folder!(), "palia.usmap"));
...
fn unversioned() -> Result<(), Error> {
    for (asset_data, bulk_data) in TEST_ASSETS {
        let mut parsed = Asset::new(
            ...
            Some(Usmap::new(Cursor::new(USMAP))?),
        )?;
